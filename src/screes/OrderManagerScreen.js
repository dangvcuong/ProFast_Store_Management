    ...
    useEffect(() => {
        fetchOrders((data) => {
            ...
            const ordersArray = Object.keys(data).map(key => {
                ...
                console.log("Đơn hàng:", order);  // Log từng đơn hàng
                return order;
            });
            ...
            const sortedOrders = ordersArray.sort((a, b) => {
                ...
                // Kiểm tra xem dateA và dateB có hợp lệ không
                if (isNaN(dateA.getTime()) || isNaN(dateB.getTime())) {
                    console.error("Ngày không hợp lệ:", a.createdAt, b.createdAt);
            ...
            // Cập nhật state với danh sách đơn hàng đã sắp xếp
                    ...
                    {filteredOrders.length > 0 ? (
                        filteredOrders.map((order, index) => (
                            <tr key={order.orderId}>
                                ...
                                <td>{order.totalAmount > 0 ? 'Tiền mặt' : 'Ví Grofast'}</td>
                ...
                <div className="confirmation-dialog">
                    <div style={{ width: 500, height: 100, backgroundColor: "white", textAlign: 'center', borderRadius: 10, padding: 10 }}>
                        ...
                        <div className="dialog-footer">
                            <button onClick={() => { handleConfirmStatus(currentOrderId); closeDialog(); }} style={{ color: 'white', backgroundColor: '#2196F3' }}>Đồng ý</button>
                            <button onClick={closeDialog} style={{ color: 'white', backgroundColor: '#2196F3' }}>Không</button>
                        </div>
                    </div>
                </div>
            )}
            {confirmDialogdl && (
                <div className="confirmation-dialog">
                    <div style={{ width: 500, height: 100, backgroundColor: "white", textAlign: 'center', borderRadius: 10, padding: 10 }}>
                        ...
                        <div className="dialog-footer">
                            <button onClick={() => { handledelivery(currentOrderId); closeDialog(); }} style={{ color: 'white', backgroundColor: '#2196F3' }}>Đồng ý</button>
                            <button onClick={closeDialog} style={{ color: 'white', backgroundColor: '#2196F3' }}>Không</button>
                        </div>
                    </div>
                </div>
            ...
            {confirmDialogSx && (
                <div className="confirmation-dialog">
                    <div style={{ width: 500, height: 100, backgroundColor: "white", textAlign: 'center', borderRadius: 10, padding: 20 }}>
                        ...
                        <div className="dialog-footer">
                            <button onClick={() => { handleMarkAsDelivered(currentOrderId); closeDialog(); }} style={{ color: 'white', backgroundColor: '#2196F3' }}>Đồng ý</button>
                            <button onClick={closeDialog} style={{ color: 'white', backgroundColor: '#2196F3' }}>Không</button>
                        </div>
                    </div>
                </div>
            ...
            {cancelDialog && (
                <div className="confirmation-dialog">
                    <div style={{ width: 500, height: 100, backgroundColor: "white", textAlign: 'center', borderRadius: 10, padding: 20 }}>
                        ...
                        <div className="dialog-footer">
                            <button onClick={() => { handleCancelStatus(currentOrderId); closeDialog(); }} style={{ color: 'white', backgroundColor: '#2196F3' }}>Đồng ý</button>
                            <button onClick={closeDialog} style={{ color: 'white', backgroundColor: '#2196F3' }}>Không</button>
                        </div>
                    </div>
                </div>
